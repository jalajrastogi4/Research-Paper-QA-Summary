...
from core.db import get_session, db_manager
...
class HealthCheck:
        ...
        try:
            ...
            if not workers:
                ...
                try:
                    ...
                    logger.warning("No celery workers found, but Redis is reachable")
    ...
    async def check_pinecone(self) -> bool:
        """
        Health check for Pinecone vector store.
        
        Verifies:
        1. Pinecone client can be initialized
        2. Index exists and is accessible
        3. Can retrieve index statistics
        
        Returns:
            True if Pinecone is accessible and healthy, False otherwise
        """
        try:
            from utils.vector_store import VectorStoreManager
            
            vector_store_manager = VectorStoreManager()
            index_stats = vector_store_manager.get_index_stats()
            if not index_stats:
                return False
            self._last_check["pinecone"] = datetime.now(timezone.utc)
            return True
        except Exception as e:
